...
function main(buildfile = "glade/dcellc.glade")
  ...
  thresholdslider = GtkReactive.slider(10:100, value = 30, 
                                       widget=build["thresholdSlider"])

  mergedistslider = GtkReactive.slider(0:0.1:20, value = 0, 
  ...
  trainmodelbutton  = GtkReactive.togglebutton(widget=build["trainModelButton"])
  ...
  showdensity       = GtkReactive.togglebutton(widget=build["densityToggleButton"])
  ...
  runcanvases(imgcanvas, lblcanvas, currentlbl, currentdens,
              currentframe, currentzr, config, history, signal(showdensity))
  ...
  initapplymodel(currentmodel, currentdens, currentframe, 
                 applymodelbutton, mainprogressbar, history)
